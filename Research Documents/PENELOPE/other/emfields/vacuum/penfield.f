...
C
C  *********************************************************************
C              SUBROUTINE PACKAGE PENFIELD   (version 2005)
C  *********************************************************************
C
...
C
C  ****  The field is set equal to zero in regions where MAT=0,
C        i.e. in the outher vacuum, since otherwise particles
C        that leave the system could return to it under the
C        action of the field.
C
      IF(MAT.EQ.0) THEN
        EX=0.0D0
        EY=0.0D0
...
C
      ...
      COMMON/UFIELD/EFX,EFY,EFZ,BFX,BFY,BFZ
C
C  ****  Same comment as in subroutine GETEMF.
C
      IF(MAT.EQ.0) THEN
        PHI=0.0D0
        RETURN
